...
exec(char *path, char **argv)
{
  ...
  char cow1[] = "test8-cow1";
  char cow2[] = "test9-cow2";
  char cow3[] = "test10-cow3"; 
  /* CSE 536: (2.1) Check on-demand status. */
  if(p->pid > 2) {
    p->ondemand = true;
  }
  if(strncmp(path, cow1,strlen(path)) == 0 || strncmp(path, cow2,strlen(path)) == 0 || strncmp(path, cow3,strlen(path)) == 0) {
    p->ondemand = false;
  }
  if (p->ondemand == true) {
  ...
  end_op();
  ip = 0;
  // printf("process getting executed 1");
  p = myproc();
  uint64 oldsz = p->sz;
  // printf("\n %s process name \n", p->name);
