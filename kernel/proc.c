...
freeproc(struct proc *p)
{
  ...
  if(p->pagetable){
    if(p->cow_enabled == 1){
      proc_freepagetable_cow(p->pagetable, p->sz, p->cow_group);
    } else {
...
//COW alternative
void
proc_freepagetable_cow(pagetable_t pagetable, uint64 sz, int cow_group)
{
  uvmunmap(pagetable, TRAMPOLINE, 1, 0);
  uvmunmap(pagetable, TRAPFRAME, 1, 0);
...
growproc(int n)
{
  ...
  n = PGROUNDUP(n);
  // printf("\n GROWPROC: processname = %s, pid = %d ,old-size = %d, addition = %d, new size = %d\n", p->name, p->pid, p->sz, n, p->sz + n);
  sz = p->sz;
  if(n > 0){

    if(p->ondemand == true){
      for(int i = sz; i<p->sz+n; i+=PGSIZE){
      int j;
      bool found = false;
      bool alreadyPresent = false;

      for (j = 0; j < MAXHEAP; j++) {
            // printf("\nGROWPROC: index -> %d, p->heap_tracker[%d].addr -> %x\n", j,j,p->heap_tracker[j].addr);
                if(p->heap_tracker[j].addr == sz) {
                  alreadyPresent = true;
                  break;
                }
                if (p->heap_tracker[j].addr == 0 || p->heap_tracker[j].addr == 0xFFFFFFFFFFFFFFFF) {
                   found = true;
          ...
          if(start == -1){
            ...
            // if (i == MAXHEAP) {
            //     return -1; // No empty slot in the heap tracker, cannot allocate
            // }
          
          p->heap_tracker[j].addr = (uint64) PGROUNDDOWN(i);
          sz+=PGSIZE;
          
          // printf("\n heap tracker -> %x, heap_tracker_index -> %d, fa_pgdown-> %x\n",p->heap_tracker[j].addr, j, PGROUNDDOWN(i));
    }
      print_skip_heap_region(p->name, p->heap_tracker[start].addr, (n)/PGSIZE);
      p->sz = sz;
      // printf("\nGROWPROC: reached return 0 -> %d\n", p->sz);
      return 0;
    }
    if(p->ondemand == false && (sz = uvmalloc(p->pagetable, sz, sz + n, PTE_W)) == 0) {
  ...
  // printf("\nGROWPROC: static alloc pre assignment psize-> %d and size -> %d\n", p->sz, sz);
  p->sz = sz;
  // printf("\nGROWPROC: static alloc post assignment psize-> %d and size -> %d\n", p->sz, sz);
  return 0;
}
...
if((uvmcopy(p->pagetable, np->pagetable, p->sz) < 0)){
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->state = RUNNABLE;
  release(&np->lock);
  // printf("\nFORK: parent - %d, child - %d, cow - %d\n", p->pid, pid, cow_enabled);
  return pid;
}
